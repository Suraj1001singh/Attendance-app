...
const [newOption, setNewOption] = useState("");
  ...
  return (
    <FormControl id={label.toLowerCase()}>
        ...
        {(options && options.length > 0) ? (<Stack pl={6} mt={1} spacing={1} overflow="auto" maxHeight={"120px"} borderWidth="1px" borderColor="light-grey">
        ...
        </Stack>) : (<Input placeholder={`No ${label} added!`} _placeholder={{ opacity: 1, color: 'black' }} disabled></Input>)}
        
        <Button colorScheme="primary" size="sm" margin="10px" float={"right"} onClick={() => setIsAddOptionOpen(true)}>
            Add {label}
        </Button>
